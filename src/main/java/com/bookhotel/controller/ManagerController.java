...
import com.bookhotel.request.AddressRequest;
import com.bookhotel.request.ServiceRequest;
import com.bookhotel.response.CommentResponse;
import com.bookhotel.response.OrderResponse;
...
public class ManagerController {
    ...
    @Operation(summary = "Lấy khách sạn sau khi đăng nhập bằng quyền manager", description = "Trả về khách sạn theo manager", tags = {"Manager Controller -HOTEL"})
    ...
    ResponseEntity<ResponseObject> createRoom(@PathVariable(value = "hotelId") int hotelId, @RequestBody Room newRoom) {
        ...
        if (foundHotelId.isPresent()) {
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("ok", "Query room successfully", foundHotelId));
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseObject("failed", "Cannot find hotel", ""));
        }
    }
    ...
    ResponseEntity<ResponseObject> updateRoom(@PathVariable(value = "roomId") int roomId, @RequestBody Room roomRequest) {
        ...
        if (updateRoom.isPresent()) {
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("ok", "Update Room Successfully", updateRoom));
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseObject("failed", "Cannot find room", ""));
        }
    }
    ...
    ResponseEntity<ResponseObject> deleterRoom(@PathVariable(value = "roomId") int roomId) {
        ...
        if (exists) {
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("ok", "Delete Room Successfully", ""));
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseObject("failed", "Cannot find room to delete", ""));
        }
    ...
    ResponseEntity<ResponseObject> createRoomImage(@PathVariable(value = "roomId") int roomId, @RequestBody RoomImage newRoomImage) {
        ...
        if (foundRoomId.isPresent()) {
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("ok", "Query room image successfully", foundRoomId));
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseObject("failed", "Cannot find room", ""));
        }
    }
    ...
    ResponseEntity<ResponseObject> updateRoomImage(@PathVariable(value = "roomId") int roomId, @PathVariable(value = "roomImageId") int roomImageId, @RequestBody RoomImage roomImageRequest) {
        if (!roomRepository.existsById(roomId)) {
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("ok", "Update Room Image Successfully", updateRoomImage));
        }
    }
    ...
    ResponseEntity<ResponseObject> deleterRoomImage(@PathVariable(value = "roomId") int roomId, @PathVariable(value = "roomImageId") int roomImageId) {
        if (!roomRepository.findById(roomId).isPresent()) {
            ...
            if (exists) {
                ...
                return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("ok", "Delete Room Image Successfully", ""));
            } else {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseObject("failed", "Cannot find room image to delete", ""));
            }
        }
    }
    ...
    ResponseEntity<ResponseObject> updateAddressHotel(@PathVariable(value = "hotelId") int hotelId, @RequestBody AddressRequest addressRequest) {
        ...
        return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("ok", "Update Address Hotel Successfully", updateHotel));
    ...
    ResponseEntity<ResponseObject> createService(@PathVariable(value = "hotelId") int hotelId, @RequestBody RoomService newRoomService) {
        ...
        if (foundHotelId.isPresent()) {
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("ok", "Query services successfully", foundHotelId));
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseObject("failed", "Cannot find hotel", ""));
        }
    }
    ...
    ResponseEntity<ResponseObject> updateRoomService(@PathVariable(value = "hotelId") int hotelId, @PathVariable(value = "serviceId") int serviceId, @RequestBody RoomService roomServiceRequest) {
        if (!hotelRepository.existsById(hotelId)) {
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("ok", "Update Service Successfully", updateRoomService));
        }
    }
    ...
    ResponseEntity<ResponseObject> deleterService(@PathVariable(value = "hotelId") int hotelId, @PathVariable(value = "serviceId") int serviceId) {
        if (!hotelRepository.findById(hotelId).isPresent()) {
            ...
            if (exists) {
                ...
                return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("ok", "Delete Service Successfully", ""));
            } else {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseObject("failed", "Cannot find service to delete", ""));
            }
        }
    }

    @Operation(summary = "Thêm các dịch vụ vào phòng của khách sạn bằng quyền manager", description = "Trả về thêm thành công hay thất bại", tags = {"Manager Controller -ROOM"})
    @PreAuthorize("hasRole('MODERATOR')")
    ...
    ResponseEntity<ResponseObject> addServicesToRoom(@PathVariable(value = "roomId") int roomId, @RequestBody ServiceRequest serviceRequest) {
        ...
        if (strServices.size() != 0) {
            strServices.forEach(service -> {
                RoomService foundService = roomServiceRepository.findByName(service).orElse(null);
                services.add(foundService);
            });
        }
        Room foundRoomId = roomRepository.findById(roomId).map(room -> {
    ...
    ResponseEntity<ResponseObject> confirmOrder(@PathVariable(value = "roomOderId") int roomOderId) {
        ...
        return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("ok", "Confirm Room Order successfully", foundRoomOrderId));
    ...
    public List<CommentResponse> getCommentsByHotel(@PathVariable(value = "hotelId") int hotelId) {
        ...
        listRoom.forEach(room -> {
            ...
            comments.forEach(comment -> {
                ...
                commentResponse.setUsername(comment.getRoomOrder().getUser().getUsername());
                commentResponse.setEmail(comment.getRoomOrder().getUser().getEmail());
    ...
    ResponseEntity<ResponseObject> deleteComment(@PathVariable(value = "commentId") int commentId) {
        ...
        if (exists) {
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new ResponseObject("ok", "Delete Comment Successfully", ""));
        } else {
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new ResponseObject("failed", "Cannot find comment to delete", ""));
        }
